...
router.get('/', auth(), (req, res) => {
    const slots = db.prepare('SELECT * FROM appointment_slots ORDER BY date ASC, start_time ASC').all();
    res.json(slots);
});

// POST /api/appointment-slots/generate — bulk generate slots for a date range
router.post('/generate', auth(['admin']), (req, res) => {
    ...
    const slotDuration = duration || 30; // minutes (changed from 60 for hospital appointments)
    ...
    while (current <= end) {
        ...
        while (hour < endH) {
            ...
            const existing = db.prepare('SELECT id FROM appointment_slots WHERE date = ? AND start_time = ?').get(dateStr, startTime);
            if (!existing) {
                db.prepare(
                    'INSERT INTO appointment_slots (id, date, start_time, end_time, duration, is_break) VALUES (?, ?, ?, ?, ?, ?)'
...
// POST /api/appointment-slots — single slot
router.post('/', auth(['admin']), (req, res) => {
    ...
    const id = uuidv4();
    db.prepare('INSERT INTO appointment_slots (id, date, start_time, end_time, duration, is_break) VALUES (?, ?, ?, ?, ?, ?)')
        .run(id, date, start_time, end_time, duration || 30, is_break ? 1 : 0);
    res.status(201).json({ id, date, start_time, end_time });
});

// DELETE /api/appointment-slots/:id
router.delete('/:id', auth(['admin']), (req, res) => {
    db.prepare('DELETE FROM appointment_slots WHERE id = ?').run(req.params.id);
